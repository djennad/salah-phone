...
function createApp(db, opts = {}) {
  ...
  // --- vider le catalogue (produits, catégories, marques, modèles) ---
  admin.post('/catalog/wipe', (req, res) => {
    if (req.body.confirm !== 'SUPPRIMER') throw bad('confirm_required');
    const count = (t) => db.prepare(`SELECT COUNT(*) AS n FROM ${t}`).get().n;
    const deleted = { products: count('products'), categories: count('categories'), brands: count('brands'), models: count('models') };
    tx(db, () => {
      // Les commandes gardent le nom et le prix des articles (order_items.product_id passe à NULL).
      db.exec('DELETE FROM product_models; DELETE FROM products; DELETE FROM models; DELETE FROM brands; DELETE FROM categories;');
    });
  ...
  admin.get('/users', (req, res) => {
    ...
    if (req.query.type === 'admin') conds.push('u.is_admin = 1');
  ...
  admin.put('/users/:id', (req, res, next) => {
    ...
    if (typeof b.is_admin === 'boolean') {
      // Un administrateur ne peut pas retirer ses propres droits (évite de se bloquer hors du site)
      if (id === req.user.id && !b.is_admin) throw bad('cannot_change_self');
      isAdmin = b.is_admin ? 1 : 0;
    }
    db.prepare('UPDATE users SET type = ?, pro_status = ?, is_admin = ? WHERE id = ?').run(type, proStatus, isAdmin, id);
    res.json({ ok: true });
  });

  // Ajouter un administrateur : promeut un compte existant, ou crée un nouveau compte admin.
  admin.post('/admins', (req, res) => {
    ...
    const password = String(b.password || '');
    if (!firstName || !lastName) throw bad('name_required');
    if (!PHONE_RE.test(phone)) throw bad('phone_invalid');
    if (password.length < 8 || password.length > 200) throw bad('password_short');
    db.prepare(`INSERT INTO users (first_name, last_name, email, password_hash, phone, is_admin) VALUES (?,?,?,?,?,1)`)
      .run(firstName, lastName, email, hashPassword(password), phone);
  ...
  // Pages, scripts et styles : le navigateur revérifie à chaque visite (réponse 304 si rien n'a changé),
  // pour que chaque mise à jour du site soit visible immédiatement.
  const noCache = (res) => res.setHeader('Cache-Control', 'no-cache');
  app.use(express.static(publicDir, { index: false, setHeaders: noCache }));
  app.get('/admin', (req, res) => res.sendFile(path.join(publicDir, 'admin.html'), { headers: { 'Cache-Control': 'no-cache' } }));
  app.get('*', (req, res) => res.sendFile(path.join(publicDir, 'index.html'), { headers: { 'Cache-Control': 'no-cache' } }));
